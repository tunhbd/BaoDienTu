    ...
    for (let index = startPos; index < endPos; index++) {
      let postItem = $(
        `<tr class="post-list__row" data-toggle="modal" data-target="#postDetail">
          <td class="post-list__cell">${postsList[index].post_title}</td>
        ...
        (postsList[index].author.pseudonym === undefined || postsList[index].author.pseudonym === '')
          ? postsList[index].author.user_fullname
      ...
      postItem.append(renderControlTooltip(postsList[index].post_id, index, postItem))
      postItem.click(() => {
...
function generatePostList(dataList) {
  ...
  if (PAGES[currentDashboardPage].status) {
    dataList.forEach(data => {
        ...
        `<tr class="post-list__row" data-toggle="modal" data-target="#postDetail">
          <td class="post-list__cell post-list__cell-choose">
            <div class="custom-control custom-checkbox d-flex align-content-center">
              <input type="checkbox" class="custom-control-input bao-dien-tu-checkbox" post-id="${data.post_id}" id="checkbox-post-${data.id}">
              <label class="custom-control-label bao-dien-tu-checkbox-mark" for="checkbox-post-${data.post_id}"></label>
            </div>
          </td>
          <td class="post-list__cell">${data.post_title}</td>
          <td class="post-list__cell">${data.category.category_name}</td>
          <td class="post-list__cell">
            ${
        (data.author.pseudonym === undefined || data.author.pseudonym === '')
          ? data.author.user_fullname
      ...
      postItem.append(renderControlTooltip(data.post_id, data, postItem))
      postItem.click(() => {
        showPostDetail(data)
        showCheckingFunction()
      })
