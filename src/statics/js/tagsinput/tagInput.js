  ...
  return this.each(function () {
    ...
    switch (action) {
      case 'init':
        ...
        if (hiddenInput.val() != "") {
          var defaultValues = JSON.parse(hiddenInput.val());
    ...
    function closeLabel(id, tagId = null) {
      tagId !== null && $(`.tag-hint[tag-id="${tagId}"]`).show() && $(`.tag-hint[tag-id="${tagId}"]`).removeAttr('hidden')
      if (id > 0) {
      ...
      hiddenInput.val(JSON.stringify(JSON.parse(hiddenInput.val()).filter(function (tag) {
    ...
    function addLabel(tag) {
      ...
      if (tagInput.children('span.tagLabel').length > 0) {
        ...
        label = $('<span class="' + settings.labelClass + ' tagLabel" data-badge="' + id + '" tag-id="' + tag.tagId + '">' + tag.tagName + ' <a href="#" data-badge="' + id + '" aria-label="close" class="closelabel">&times;</a></span> ').insertAfter(badge);
      } else {
        label = $('<span class="' + settings.labelClass + ' tagLabel" data-badge="1" tag-id="' + tag.tagId + '">' + tag.tagName + ' <a href="#" data-badge="1" aria-label="close" class="closelabel">&times;</a></span> ').insertBefore(textInput);
      }
