...
const getAllDetailUserFilterBy = (role, pageNum) => new Promise((resolve, reject) => {
  let query =
    `SELECT user_account, user_fullname, user_email, user_birthday, user_avatar, user_status, user_role 
    FROM users
  ...
  dbConn
    ...
    .then(async rets => {
      let users = rets.map(ret => {
        let user = ret.user_role === USER_ROLES.SUBSCRIBER ? new Subscriber() : ret.user_role === USER_ROLES.WRITER ? new Writer() : ret.user_role === USER_ROLES.EDITOR ? new Editor() : new User()
...
const getCountAllUserFilterBy = (role) => new Promise((resolve, reject) => {
  let query = `SELECT COUNT(*) count FROM users ${role !== 'ALL' ? `WHERE user_role='${role}'` : ''}`
  let dbConn = new DBConnection()
...
const getExpirationDateOfSubscriber = account => new Promise((resolve, reject) => {
...
const getAssignedCategoriesOfEditor = account => new Promise((resolve, reject) => {
  let query =
