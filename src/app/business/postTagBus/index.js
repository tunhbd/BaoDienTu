...
const deletePostTagsByTagIds = tagIds => new Promise((resolve, reject) => {
  let query = `DELETE FROM post_tags WHERE tag_id IN (${tagIds.map(id => `'${id}'`).join(',')})`
  let dbConn = new DBConnection()
