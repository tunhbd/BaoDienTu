...
const getFullInfoTags = () => new Promise((resolve, reject) => {
  ...
  DBConnect
    ...
    .then(ret => {
      ...
      ret.forEach(tag => {
        tags.push((new Tag(tag.tag_id, tag.tag_name, tag.tag_active, tag.created_date)))
      });
...
const getFullInfoTagsFilterBy = (pageNum) => new Promise((resolve, reject) => {
  let queryString = `SELECT * FROM tags LIMIT ${(pageNum - 1) * LIMIT_TAGS}, ${LIMIT_TAGS}`
  let DBConnect = new DBConnection()
...
const updateTag = (tag) => new Promise(async (resolve, reject) => {
  tag.generateAlias()

  let queryString = `UPDATE tags SET tag_name='${tag.tagName}', tag_alias='${tag.alias}' WHERE tag_id='${tag.tagId}'`

  let DBConnect = new DBConnection()

  DBConnect
    .updateRequest(queryString)
    .then(ret => {
      resolve(ret ? tag : {})
    })
    .catch(err => {
...
const deleteTags = tagIds => new Promise((resolve, reject) => {
  postTagBus
    .deletePostTagsByTagIds(tagIds)
    .then(ret => {
      let query = `DELETE FROM tags WHERE tag_id IN (${tagIds.map(id => `'${id}'`).join(',')})`
      let dbConn = new DBConnection()
      dbConn
        .deleteRequest(query)
        .then(ret => {
...
const getCountTags = () => new Promise((resolve, reject) => {
