...
const signUp = (req, res, next) => {
  if (req.user) {
    ...
    authBus
      ...
      .catch(err => {
        console.log(err)
        req.flash('mes', 'Hiện đang có mốt số lỗi xảy ra trên server. bạn chưa thể thực hiện đăng ký bây giờ. Bạn có thể thửu đăng ký vào thời gian khác.');
      ...
      .then(ret => {
        if (confirmPassword !== password) {
          ...
          authBus
            ...
            .catch(err => {
              req.flash('mes', 'Xảy ra lỗi trên server. Bạn có thể thử lại vào lần sau')
              res.render("user/changePassword", {
                data: {
                  user: req.user,
                  ov: info,
...
const updateProfile = (req, res, next) => {
  if (!req.user) {
    ...
    user.birthday = moment(req.body.birthday, 'DD/MM/YYYY').format('YYYY/MM/DD')
    user.role === config.USER_ROLES.WRITER ? user.pseudonym = req.body.pseudonym : null
          ...
          .then(ret => {
            if (ret) {
              ...
              sgMail
                .send(msg)
                .then(ret => {
                  req.flash('mes', 'một email khôi phục mật khẩu đã được gửi đến hộp thoại mail của bạn. Hãy kiểm tra hộp thư để tiến hành khôi phục mật khẩu')
                  res.render('user/forgotPassword', {
                    data: {
                      message: { success: req.flash("mes") }
                    },
                ...
                .catch(err => {
                  console.log(err)
                  req.flash('mes', 'Hiện tại hệ thống không thể hỗ trợ bạn khôi phục mật khẩu. Bạn có thể thử lại lần sau.')
                  res.render('user/forgotPassword', {
                    data: {
                      info: req.body,
                      message: { error: req.flash("mes") }
