    ...
    if (req.isSignIn) {
      if (req.user.user_role === config.USER_ROLES.SUBSCRIBER) {
        ...
        res.render("dashboard", {
          user: { ...req.user, user_avatar: "avatar_sample.png" },
          layout: false
        });
      }
    } else {
      res.render("dashboard", {
        user: { user_avatar: "avatar_sample.png" },
        layout: false
      });
    }
  } else {
...
const getEditPostUIGetRequest = (req, res) => {
  res.render("templates/dashboard-uis/editPostUI", {
    categories: mockData.CATEGORIES_LIST,
    layout: false
  });
};

const getEditUserUIByRoleGetRequest = (req, res) => {
  res.render("templates/dashboard-uis/editUserUI", {
    ...
    if (!req.isSignIn) {
      ...
      switch (req.params.pageId) {
        case config.PAGES.CREATE_POST:
          Promise.all([
            ...
            .then(([categories, tags]) => {
              console.log("DONE");
              res.render("templates/dashboard-uis/createPostUI", {
                categories,
                tags,
                layout: false
              });
        ...
        case config.PAGES.DRAFT:
          res.render("templates/dashboard-uis/postsListUI", {
            categories: mockData.CATEGORIES_LIST,
            status: false,
            layout: false
          });
        ...
        case config.PAGES.CATEGORY:
          res.render("templates/dashboard-uis/categoriesListUI", {
            layout: false
          });
          break;
